...
pair<vector<char>, int> branchAndBoundHeuristic(const unordered_map<char, unordered_map<char, int>>& graph, 
    ...
    while (!queue.empty()) {
        ...
        if (cost + heuristics.at(node) >= best_cost) {
            continue;
        }
...
int main() {
    ...
    auto result = branchAndBoundHeuristic(graph, 'S', 'G', heuristics);
